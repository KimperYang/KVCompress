...
for i in range(total_num):
    ...
    for j in range(0,10):
        ...
        segment_ids_1.extend([j+1] * (len(tem_id) + len(compress_tokens)))
        segment_ids_2.extend([1] * len(tem_id) + [2] * len(compress_tokens))
        position_ids.extend(list(range(current_index - len(tem_id), current_index + len(compress_tokens))))
