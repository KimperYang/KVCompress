...
for i in range(total_num):
    ...
    for j in range(len(data[i]['documents'])):
        ...
        chunk_compress_token_len = round_up_to_10(len(tem_id) * compress_ratio)
        if chunk_compress_token_len > len(compress_tokens):
            chunk_compress_token_len = len(compress_tokens)
        chunk_compress_tokens = compress_tokens[:chunk_compress_token_len]
        ...
        input_ids.extend(tem_id + chunk_compress_tokens + link_tokens[j])
...
file_name = f"result/prompt/{run_name}/hqa2_full_ckpt{ckpt}_{accuracy}.jsonl"
