...
data=datasets.load_dataset("hotpotqa/hotpot_qa", 'distractor', split='validation')
...
for i in range(total_num):
    ...
    for j in range(len(data[i]['context']['title'])):
        ...
        for idx in range(0, len(tem_id), chunk_size):
            ...
            if len(chunk_id) < chunk_size:
                chunk_id.extend([pad_token] * (chunk_size - len(chunk_id)))
            
            segment_ids_1.extend([chunk_idx] * (chunk_size + 1 + len(compress_tokens)))
            segment_ids_2.extend([1] * (chunk_size + 1) + [2] * len(compress_tokens))
            position_ids.extend(list(range(current_index - chunk_size - 1, current_index + len(compress_tokens))))
...
file_name = f"result/{run_name}/hqa_ckpt{ckpt}_{accuracy}.jsonl"
