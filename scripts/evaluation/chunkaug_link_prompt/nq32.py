...
model = AutoModelForCausalLM.from_pretrained(f"training_res/{run_name}/checkpoint-{ckpt}", torch_dtype=torch.float32)
model.to('cuda')
...
if not os.path.exists(f"result/fp32/{run_name}"):
    os.makedirs(f"result/fp32/{run_name}")

accuracy = correct_num / total_num

file_name = f"result/fp32/{run_name}/nq_{pos}_ckpt{ckpt}_{accuracy}.jsonl"
